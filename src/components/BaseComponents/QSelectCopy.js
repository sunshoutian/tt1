...
export default Vue.extend({
  ...
  watch: {
    ...
    options: {
      handler(options) {
        const optionsLength =
          Array.isArray(options) === false ? 0 : options.length
  ...
  computed: {
    fieldClass() {
      return `q-select q-field--auto-height q-select--with${
    ...
    optionScope() {
      return this.options
        .slice(this.optionsSliceRange.from, this.optionsSliceRange.to)
        .map((opt, i) => {
  ...
  methods: {
    ...
    add(opt, unique) {
      const val = this.emitValue === true ? this.__getOptionValue(opt) : opt
    ...
    toggleOption(opt) {
      ...
      const model = [].concat(this.value),
        index = this.value.findIndex(v =>
          isDeepEqual(this.__getOptionValue(v), optValue)
        )
    ...
    setOptionIndex(index) {
      ...
      const val = index > -1 && index < this.options.length ? index : -1
    ...
    __getOption(value) {
      return (
        this.options.find(opt =>
          isDeepEqual(this.__getOptionValue(opt), value)
        ) || value
      )
    },
    ...
    __isSelected(opt) {
      ...
      return (
        this.innerValue.find(v =>
          isDeepEqual(this.__getOptionValue(v), val)
        ) !== void 0
      )
    },
    ...
    __hydrateOptions(ev, toIndex) {
      ...
      if (
        ev === void 0 ||
        (this.preventNextScroll === true && toIndex === void 0)
      ) {
        return
      }

      const delayNextScroll =
          this.delayNextScroll === true && toIndex === void 0,
        target =
          delayNextScroll === true ||
          ev.target === void 0 ||
          ev.target.nodeType === 8
            ? void 0
            : ev.target,
        content =
          target === void 0
            ? null
            : target.querySelector('.q-select__options--content')
      ...
      const scrollTop = target.scrollTop,
        ...
        childPosTop =
          child === void 0 ? -1 : content.offsetTop + child.offsetTop,
        childPosBottom =
          child === void 0 ? -1 : childPosTop + child.clientHeight,
      ...
      if (fromScroll === true) {
        ...
        toIndex = -1
        for (
          let i = Math.trunc(scrollTop + viewHeight / 2);
          i >= 0 && toIndex < toIndexMax;
    ...
    __setOptionsSliceRange(toIndex, target, fromScroll) {
      const from = Math.max(
          0,
          Math.min(
            toIndex - Math.round(optionsSliceSize / 2),
            this.options.length - optionsSliceSize
          )
        ),
        to = from + optionsSliceSize,
        repositionScroll =
          fromScroll !== true || from < this.optionsSliceRange.from

      if (
        from === this.optionsSliceRange.from &&
        to === this.optionsSliceRange.to
      ) {
        if (fromScroll === true) {
          return
        }
      ...
      this.$nextTick(() => {
        ...
        const marginTop = this.optionsHeights
            .slice(from)
            .reduce((acc, h) => acc + h, 0),
          height =
            marginTop +
            this.optionsHeights.slice(0, from).reduce((acc, h) => acc + h, 0),
          padding =
            (this.optionsHeight % optionsListMaxPadding) +
            height -
            this.optionsHeight
        ...
        if (
          ...
          content.previousSibling.style.cssText =
            padding >= 0
              ? `height: ${padding}px; margin-top: 0px`
              : `height: 0px; margin-top: ${padding}px`
          ...
          if (fromScroll !== true) {
            ...
            target.scrollTop =
              ...
              (this.$q.platform.is.mobile === true
                ? 0
                : Math.trunc(
                    this.optionsHeights[toIndex] / 2 - target.clientHeight / 2
                  ))
    ...
    __getSelection(h, fromDialog) {
      ...
      if (this.$scopedSlots['selected-item'] !== void 0) {
        return this.selectedScope.map(scope =>
          this.$scopedSlots['selected-item'](scope)
        )
      }
    ...
    __getControl(h, fromDialog) {
      ...
      if (
        this.useInput === true &&
        (fromDialog === true || this.hasDialog === false)
      ) {
    ...
    __getOptions(h) {
      ...
      for (
        let i = Math.trunc(this.optionsHeight / optionsListMaxPadding);
        i > 0;
        i--
      ) {
        list.push(
          h('div', {
            staticClass: 'q-select__options--padding',
            style: { height: `${optionsListMaxPadding}px` }
          })
        )
      }
      list.push(
        h('div', {
          staticClass: 'q-select__options--padding',
          style: { height: `${this.optionsHeight % optionsListMaxPadding}px` }
        })
      )
    ...
    __getInput(h) {
      ...
      return h('input', {
        ...
        class:
          this.hideSelected !== true && this.innerValue.length > 0
    ...
    __updateMenu(show) {
      ...
      if (show === true) {
        ...
        this.__hydrateOptions(
          { target: this.__getMenuContentEl() },
          optionIndex
        )
      }
  ...
  mounted() {
    this.__setOptionsSliceRange =
      this.$q.platform.is.ios === true || this.$q.platform.is.safari === true
